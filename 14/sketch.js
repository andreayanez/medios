...
var velx, vely; //variables para velocidad o cambio de posicion en el o
...
var score; //crea una lista de tiempo para guardar el tiempo de score
...
function draw() {
  ...
  if (estado == 0) {
    score = 0; //empieza el conteo de nuevo del puntaje
    background(255, 255, 204);
    push();
    noStroke();
    textSize(30);
    fill(153, 0, 0);
    //text("QUICKLY!!", width / 2, (height / 2) - 180);
    text("WELCOME TO", width / 2, (height / 2) - 70);
    text("THE IMPOSSIBLE TEST", width / 2, (height / 2) - 30);
    //text("Time is gold", width / 2, (height / 2) + 220);

    fill(76, 153, 0);
    ellipse(width / 2 - 100, (height / 2) + 100, 120, 120); //circulo verde play
    fill(255, 02, 55);
    ellipse(width / 2 + 100, (height / 2) + 100, 120, 120); //circulo verde play multiplayer

    textSize(20);
    fill(1);
    text("1 Player", width / 2 - 100, (height / 2) + 100); //texto play en circulo
    text("Multiplayer", width / 2 + 100, (height / 2) + 100); //texto play en circulo
    pop();
  }
  ...
  if (estado > 0) { //&& estado <21
    ...
    textSize(20);
    fill(1);
    text('Score:', width / 4 - 50, height / 4);
    text(score, width / 4, height / 4);
    //text(floor(tiempoInicial / 1000), width / 4, height / 4);
  ...
  if (estado == 12) {
    ...
    text("it's actually right", width / 2, (height / 2) - 60);
    pop();
  ...
  if (estado == 16) {
    ...
    text("question 20 is 20", width / 2, (height / 2) - 50);
    pop();
  ...
  //NIVEL 18 YELL TO ADVANCE
  if (estado == 18) {
    //Instrucciones
    push();
    textSize(40);
    fill(153, 0, 0);
    text("Yell to advance", width / 2, (height / 2) - 100);
    pop();

    // Get the overall volume (between 0 and 1.0)
    var vol = mic.getLevel() * 3400;
    fill(127);
    stroke(0);

    // Draw an ellipse with height based on volume
    var h = map(vol, 0, 3400, height, 0);
    ellipse(width / 2, h - 375, 150, 150);
    print(mic.getLevel());
  ...
  if (estado == 20) {
    ...
    //fill(1);
    //ellipse(width / 2 +120, (height / 2) -50, 60, 40);
    //Instrucciones
    push();
    textSize(40);
    fill(153, 0, 0);
    text("What was the answer", width / 2, (height / 2) - 100);
    text("to Question 20?", width / 2, (height / 2) - 50);
    pop();
    ...
    text("21", width / 2 - 100, height / 2 + 120); //ixq down    ESTA ES !!
    text("19", width / 2 + 100, height / 2 + 50); //dere up
  ...
  if (estado == 98) {
    ...
    input.position(width / 2 - 100, height / 4 + 65);
    button.position(input.x + input.width, height / 4 + 65);
    button.mousePressed(greet);
    greeting.position(width / 2 - 120, height / 4 + 5);
    ...
    //este sirve pero no lo voy a usar//
    /*/
    text("Final Score", width/2-100, height/4);
  ...
  //tiempo[1] = tiempo en el que se hizo el primer clic
  //tiempo[0] = tiempo en el que se hizo el segundo clic
  var score = 60000 / (tiempo[1] - tiempo[0]);
...
function mousePressed() {
  ...
  if (estado == 0) {
    background(255, 255, 204);
    if (dist(mouseX, mouseY, width / 2 - 100, (height / 2) + 100) < 50) {
      estado = 1; //PASA A NIVEL 1
      lives = 10;
      score++;
    } else if (dist(mouseX, mouseY, width / 2 + 100, height / 2 + 100) < 60) {
      estado = 1;
      lives = 10;
      score++;
    }
  ...
  //nivel 17 touch 5 fingers
    ...
    for (var j = 4; j < touches.length; j++) { //como es un arreglo 4 guarda 5 posiciones
      estado = 18; //PASA A NIVEL 18
      score++;
    }
  }
  ...
  //NIVEL answer to q20 =20
  else if (estado == 20) {
    if (dist(mouseX, mouseY, width / 2 + 120, (height / 2) - 50) < 30) { //IZQ DOWN NO ES
      estado = 21; //PASA A NIVEL 21
      score++;
    }
    if (dist(mouseX, mouseY, width / 2 - 100, height / 2 + 120) < 30) { //IZQ DOWN NO ES
      lives = lives - 1;
...
//funcion que se llama cuando se hace clic touchStarted()
function mouseClicked() { //touchStarted()
...
function greet() {
  ...
  if (estado == 98) {
    ...
    greeting.html(' ' + name + ' your final score is: ' + score);
    input.value('');
